...
void AFPRPGCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind fire event
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AFPRPGCharacter::OnFire);

	// Enable touchscreen input
	EnableTouchscreenMovement(PlayerInputComponent);

	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AFPRPGCharacter::OnResetVR);

	// Bind movement events
	PlayerInputComponent->BindAxis("MoveForward", this, &AFPRPGCharacter::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &AFPRPGCharacter::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &AFPRPGCharacter::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &AFPRPGCharacter::LookUpAtRate);
}
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AFPRPGProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
...
void AFPRPGCharacter::OnResetVR()
{
...
//void AFPRPGCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
{
	if (abs(this->GetActorLocation().X != destination.X))
	{
		int Direction = (destination.X - this->GetActorLocation().X) / abs(destination.X - this->GetActorLocation().X);
		this->SetActorLocation(this->GetActorLocation() + FVector(Direction * Speed, 0, 0));
	}
	else if (this->GetActorLocation().Y != destination.Y)
	{
		int Direction = (destination.Y - this->GetActorLocation().Y) / abs(destination.Y - this->GetActorLocation().Y);
		this->SetActorLocation(this->GetActorLocation() + FVector(0, Direction * Speed, 0));
	}
...
void AFPRPGCharacter::Tick(float DeltaTime)
{
